#!/usr/bin/env python3
"""Train a DSRL-style SAC policy that selects noise for a frozen residual flow model."""
...
RESIDUAL_FLOW_ROOT = os.path.join(ROOT, "..")
DSRL_SB3_ROOT = os.path.join(ROOT, "..", "..", "..", "dsrl", "stable-baselines3")

for path in (FLOW_ROOT, RESIDUAL_FLOW_ROOT, DSRL_SB3_ROOT):
    if path not in sys.path:
        sys.path.insert(0, path)
# Always prioritize dsrl/ so local eval_viz.py wins over residual_flow/eval_viz.py.
...
from wrapper import PointMazeResidualFlowVecEnv, PointMazeResidualFlowWrapper


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to frozen base flow policy checkpoint")
    parser.add_argument(
        "--residual-checkpoint",
        type=str,
        required=True,
        help="Path to frozen residual flow policy checkpoint",
    )
    parser.add_argument(
        "--dataset-id",
        type=str,
    ...
    parser.add_argument("--sample-steps", type=int, default=20, help="Base flow policy denoising steps")
    parser.add_argument(
        "--residual-sample-steps",
        type=int,
        default=20,
        help="Residual flow denoising steps for SAC-selected noise",
    )
    parser.add_argument("--residual-scale", type=float, default=1.0)
    parser.add_argument("--action-mag", type=float, default=2.0, help="Noise action bound")
    ...
    parser.add_argument("--wandb-project", type=str, default="d4rl-residual-flow-dsrl")
...
def make_vec_env(args: argparse.Namespace, reset_options: dict[str, np.ndarray] | None) -> PointMazeResidualFlowVecEnv:
    return PointMazeResidualFlowVecEnv(
        args.n_envs,
        dataset_id=args.dataset_id,
        flow_checkpoint=args.checkpoint,
        residual_checkpoint=args.residual_checkpoint,
        flow_sample_steps=args.sample_steps,
...
def main() -> None:
    ...
    probe_env = PointMazeResidualFlowWrapper(
        dataset_id=args.dataset_id,
        flow_checkpoint=args.checkpoint,
        residual_checkpoint=args.residual_checkpoint,
        flow_sample_steps=args.sample_steps,
    ...
    residual_name = os.path.splitext(os.path.basename(args.residual_checkpoint))[0]
    run_name = args.run_name or f"{base_name}_{residual_name}"
